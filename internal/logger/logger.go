...
func GetLoggingEnv() string {
	checkRunningEnv := os.Getenv("APP_ENV")
...
func (f *myFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	...
	return []byte(fmt.Sprintf("[\u001B[%dm%s\u001B[0m][%s] - %s - [line:%d] - %s\n", levelColor,
		strings.ToUpper(entry.Level.String()), entry.Time.Format(f.TimestampFormat), fileName, entry.Caller.Line, entry.Message)), nil
}
